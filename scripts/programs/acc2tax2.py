...
for file in filelist:
	...
	path1="/".join(str(p) for p in file.split("/")[:-1])
	f=open(file,'r')
	
	
	outfile1=path1+"/"+file.split("/")[-1].split(".")[0]+".tax"
	...
	for i in p3:
		...
		if len(i.split("\t"))>1:
			...
			tax="no taxonomy found"
				
		names.append(tax) #list of lineages
	...
	for line4 in f:
		...
		if names[n].rstrip("\n")=="no taxonomy found":
			...
			nohitsuffix="no taxonomy found\tno rank__na|no rank__na|superkingdom__na|no rank__na|phylum__na|class__na|order__na|family__na|genus__na|species__"+input_species[n]+"\n"
			lineage=line4.rstrip('\n')+"\t"+gitax[n].rstrip("\n")+"\t"+nohitsuffix
			g.write(lineage)
