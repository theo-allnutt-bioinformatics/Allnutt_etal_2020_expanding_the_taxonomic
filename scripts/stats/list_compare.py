...
g.write("name\texp_n\tn\trbo\tcorrect\textra\tmissing\taccuracy\tR\tp\n")
...
for x,y in zip(expfilelist,filelist):
	...
	for x in list2:
		if x in list1 and x not in notcounttwice:
			notcounttwice.append(x)
			correct=correct+1
		else:
			extra=extra+1
	for x in list1:
	...
	extra1=extra/num
	missing1=missing/num
	correct1=correct
	em=extra1+missing1
	
	acc=float((correct-missing-extra)/num)*100
	
	output= str(len(list1))+"\t"+str(len(list2))+"\t"+str(rbo)+"\t"+str(correct1)+"\t"+str(extra)+"\t"+str(missing)+"\t"+str(acc)+"\t"+str(rho)+"\t"+str(pval)

	#"name\texp_n\tn\trbo\tcorrect\textra\tmissing\taccuracy\tR\tp\n"
